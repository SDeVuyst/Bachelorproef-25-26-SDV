\noindent\textit{Noot bij de bijlage.} Onderstaande tekst is de oorspronkelijke versie van het onderzoeksvoorstel zoals beoordeeld bij de start van de bachelorproef. Tijdens de uitvoering werd de geplande gebruikersstudie met twee focusgroepen vervangen door een vergelijkende literatuurevaluatie (zie hoofdstuk~\ref{ch:evaluatie} en de motivatie in sectie~\ref{subsec:evaluatie-keuze}); de tekst van het voorstel zelf is bewust onveranderd gelaten als historisch referentiedocument.
...
Zo kan level 1 beginnen met programmeerconcepten die gelijkaardig zijn aan Python, zoals if-statements.
Level 2 kan de globale structuur zoals de \textit{Identification, Environment, Data} en \textit{Procedure Divisions} behandelen, terwijl latere levels ingaan op variabelen, string manipulatie etc.

\subsection{Fase 3: Evaluatie en Resultaten}
Na de realisatie van de PoC wordt de efficiëntie van het instrument getoetst door middel van een gebruikersonderzoek.
...
Om de efficiëntie te meten, zal de \textit{System Usability Scale (SUS)} worden ingezet. 

...
De oorspronkelijke onderzoeksopzet voorzag een empirische vergelijking tussen de PoC en een traditioneel PDF-pack: een binnen-proefpersoon-design met counterbalancing, pre-\allowbreak/post-tests, motivatievragenlijsten, SUS en gedragslogs uit de applicatie.
...
\begin{itemize}
    ...
    \item \textbf{Materiaal en uitvoering.} Een eerlijke vergelijking vereist een vooraf gevalideerd PDF-pack, screening, gestandaardiseerde instrumenten en voor elke deelnemer twee geplande sessies plus export van applicatielogs.
\end{itemize}
...
Pilotontwerp: sectie~\ref{sec:evaluatie-pilotstudie} beschrijft een vergelijkende experiment in uitvoerbare vorm (doelgroep, steekproef, tijdslijn, meetinstrumenten en analyse), zodat een opvolger de empirische validatie kan overnemen zonder het onderzoeksplan opnieuw te moeten ontwerpen.
...
De tegenkant is een verlies aan correctheidsgarantie: de PoC kan code goedkeuren die in een echte compiler zou falen.
...
Onderstaand plan beschrijft een beknopte pilotstudie met een beperkte steekproef (\(N = 12\)--\(20\)).
Het is een haalbaarheids- en effectrichting-onderzoek.
Het doel is voorbereidende evidentie te verzamelen, de meetketen te testen en de hypotheses uit de literatuurevaluatie te evalueren met gebruikersdata.

\subsection{Doelstellingen}

De pilot heeft twee gerichte doelen.
Het eerste doel is nagaan of de PoC onder gecontroleerde omstandigheden ten minste equivalent presteert aan een statisch PDF-pack met dezelfde zeven leerthema's en volgorde als in hoofdstuk~\ref{ch:poc}, op vlak van korte-termijn leerwinst (conceptkennis) en motivatie of engagement tijdens het doorlopen van het materiaal.
Het tweede doel is om de praktische kant te bekijken: hoe gebruiksvriendelijk de app is (SUS), hoeveel deelnemers afhaken, of de planning haalbaar is, en welke objectieve gedragsgegevens we uit de app kunnen halen (zoals tijden en aantal pogingen) als aanvulling op de vragenlijsten.
...
\textbf{Steekproefgrootte:} We streven naar \(N = 12\) tot \(20\) deelnemers.
...
Voor het PDF-pack wordt een tijd-op-taak-log gevraagd (zelfrapportage of eenvoudig logformulier).
...
\begin{tikzpicture}[
    ...
    \node[lbl, below=14pt] at (0,-0.17) {\textbf{Week~1}\\[3pt]
        PDF-pack en tests afronden, informed consent, instrumenten pilot-testen buiten de steekproef.};
    ...
    \node[lbl, above=14pt] at (11.6,0.17) {\textbf{Week~5}\\[3pt]
        Optionele vertraagde test, exit-enquête, export serverlogs, pilotrapport.};
\end{tikzpicture}%
}%
...
Voordat deelnemers aan de eerste ronde beginnen, maken ze een pre-test. Deze test bevat vragen over de zeven thema's uit tabel \ref{tab:poc-levels}. 
Na elke ronde volgt een post-test met vergelijkbare, gelijkwaardige vragen. Zo wordt er direct gemeten hoeveel de deelnemer per medium heeft geleerd.
...
Een korte gestructureerde exit-enquête vervangt geen diepte-interview maar vangt toch knelpunten en verwachtingen op.